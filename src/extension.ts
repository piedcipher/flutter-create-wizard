...
export function activate(context: vscode.ExtensionContext) {
	let disposable = vscode.commands.registerCommand('extension.flutter-create-wizard',
		async () => {
			let projectName =
			...
			let androidLanguage =
				await vscode.window.showInputBox({
					...
					validateInput: (value) => {
						...
						if (value === "java" || value === "kotlin" || value === "") {
							return null;
						}
						else {
			...
			let iosLanguage =
				await vscode.window.showInputBox({
					...
					validateInput: (value) => {
						...
						if (value === "objc" || value === "swift" || value === "") {
							return null;
						}
						else {
			...
			let projectDirectory =
				await vscode.window.showOpenDialog({
					...
					.then((_projectDirectory) => {
						directoyUri = _projectDirectory === undefined ? "" : _projectDirectory[0];
			...
			exec(createCommand, (error, stdout, stderr) => {
				console.log(error);
				var newpath = path.join((<vscode.Uri>directoyUri).fsPath, folderName);
				...
				vscode.commands.executeCommand("vscode.openFolder", vscode.Uri.file(newpath), openInNewWindow);
			});
		});
